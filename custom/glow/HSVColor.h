...
namespace glow
{
  ...
  struct HSVColor
  {
    ...
    {
      uint16_t h = (hue - target.hue) * index / length % hue_limit;
      uint16_t s = (saturation - target.saturation) * index / length;
      uint16_t v = (value - target.value) * index / length;
      return HSVColor{h, (uint8_t)s, (uint8_t)v};
    }
